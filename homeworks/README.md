# 作业文件说明

## ⚠️ 重要提示

由于 GitHub Pages 的限制，所有作业 HTML 文件必须保持在**根目录**，不能移动到此文件夹中。

## 📋 作业文件列表（位于根目录）

### 作业列表和导航
- `homework.html` - 作业总列表页（主入口）
- `homework-detail.html` - 作业详情模板页
- `daily.html` - 日常作业页

### 具体作业展示
| 文件名 | 作业名称 | 说明 |
|--------|---------|------|
| `homework1.html` | 作业1 | Git 基础操作 |
| `homework2.html` | 作业2 | Arduino 流水灯 |
| `homework3.html` | 作业3 | - |
| `homework4.html` | 作业4 | - |
| `homework5.html` | 作业5 | - |
| `homework6.html` | 作业6 | - |
| `homework7.html` | 作业7 | - |
| `homework8.html` | 作业8 | - |
| `homework9.html` | 作业9 | - |

## 🔗 访问方式

所有作业页面的访问地址格式：
```
https://您的域名.github.io/zwu2026-1-004/homeworkX.html
```

例如：
- 作业1: `https://您的域名.github.io/zwu2026-1-004/homework1.html`
- 作业2: `https://您的域名.github.io/zwu2026-1-004/homework2.html`

## 📁 相关资源

作业相关的图片和视频资源存放在：
- `image/homework1/` - 作业1相关图片
- `image/homework2/` - 作业2相关图片
- `videos/homework2/` - 作业2相关视频

## 💡 为什么不能移动？

GitHub Pages 直接将根目录的文件映射到网站 URL。如果移动文件：
1. 需要修改所有引用这些文件的链接
2. 会破坏现有的导航结构
3. 可能导致页面无法访问

因此，保持所有 HTML 文件在根目录是最安全和最简单的方案。

---

最后更新：2026-06-02
